...
exports.groupDeparturesByLine = groupDeparturesByLine;
const departureFormatter_1 = require("./departureFormatter");
/**
 ...
 * - "4" -> "BUS" (default)
 */
function extractTransportMode(lineLabel) {
    if (lineLabel.startsWith('Metro '))
        return 'METRO';
    if (lineLabel.startsWith('Train '))
        return 'TRAIN';
    if (lineLabel.startsWith('Tram '))
        return 'TRAM';
    return 'BUS';
}
/**
 ...
 */
function createGroupKey(departure) {
 ...
 */
function groupDeparturesByLine(departures, maxPerLine = 3) {
    // Sort all departures by time first
    const sortedDepartures = (0, departureFormatter_1.sortDeparturesByTime)(departures);
...
//# sourceMappingURL=lineGrouper%203.js.map