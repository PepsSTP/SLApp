...
export interface RouteMatch {
  line: string;
  originStop: string;       // The stop to depart from
  destinationStop: string;  // The stop to arrive at
}

export interface DestinationGroup {
  displayName: string; // User-friendly name (e.g., "To Älvsjö Station")
  routes: RouteMatch[]; // List of origin → destination route combinations that belong to this group
}
